...
# Initialize
bot = Bot(token='8465860341:AAFYv9bkH-3ST83flC1keAQiJQof3fFDedw')
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if message.from_user.id == 8465860341:
    ...
    for i in range(count):
        ...
        if batch_state['stopped']:
            await msg.edit_text(
                ...
                reply_markup=owner_keyboard() if callback.from_user.id == 8465860341 else main_menu_keyboard()
            )
...
async def custom_email(message: Message, state: FSMContext):
    ...
    if result['success']:
        ...
        await message.answer(
            ...
            f"🔑 `{result['password']}`",
            parse_mode="Markdown",
            reply_markup=owner_keyboard() if message.from_user.id == 8465860341 else main_menu_keyboard()
        )
    else:
        await message.answer(
            f"❌ Failed\n\n"
...
async def gen_key(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != 8465860341:
        return
